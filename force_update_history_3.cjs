...
`;
  ...
  console.log('Successfully replaced normal chat history block, from ', startIndex, 'to', endIndex);
} else {
...
const groupIndices = [];
lines.forEach((l, i) => {
    if (l.includes("} else if (text.includes('联想历史合作') ") || l.includes("|| text.includes('总结过往项目')")) {
        // We will look for GroupChat
        if (l.includes("总结过往项目合作及历史数据")) return; // ignore quick skills
        if (l.includes("else if")) groupIndices.push(i);
    }
});

let startIndexGroup = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes("} else if (text.includes('联想历史合作') || text.includes('跟联想有过什么合作') || text.includes('历史合作') || text.includes('总结过往项目')) {")) {
    startIndexGroup = i;
    break;
  }
}
