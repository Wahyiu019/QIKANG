...
const regex = /\} else if \(\s*text\.includes\("烟草企业，多机构，预算池已知，历史赔付不完整，帮我准备首次拜访材料"\)\s*\) \{\s*const newUserMsg = \{\s*id: Date\.now\(\)\.toString\(\) \+ "_user",\s*sender: "user",\s*type: "text",\s*content: text,\s*timestamp: new Date\(\),\s*\};\s*setMessages\(\(prev\) => \[\.\.\.prev, newUserMsg\]\);\s*const reasoningMsgId = Date\.now\(\)\.toString\(\);\s*const allTexts = \[[\s\S]*?\} else \{\s*setMessages\(\(prev\) =>\s*prev\.map\(\(m\) =>\s*m\.id === reasoningMsgId\s*\? \{\s*\.\.\.m,\s*data: \{\s*\.\.\.m\.data,\s*steps: m\.data\.steps\.map\(\(s: any, i: number\) =>\s*i < stepIdx\s*\? \{ \.\.\.s, status: "complete" \}\s*: i === stepIdx\s*\? \{ \.\.\.s, status: "loading" \}\s*: s,\s*\),\s*\},\s*\}\s*: m,\s*\),\s*\);\s*\}\s*\}, 1000\);\s*return;\s*\}/g;

const targetMatch = cwContent.match(regex);
console.log('Matches:', targetMatch ? targetMatch.length : 0);
...
if (targetMatch) {
  ...
  cwContent = cwContent.replace(regex, newBlock);
