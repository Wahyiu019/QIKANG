...
// Insert new logic into ChatWindow
// We have two old blocks for '标书生成' and '材料审核'. Let's remove them directly first.
// The block for "帮我写一个标书" or "标书生成" is relatively large.
chatWindowContent = chatWindowContent.replace(
...
// Insert new logic right at the start of handleSend's isBot conditions
chatWindowContent = chatWindowContent.replace(
