...
const targetObj = '} else if (\n      text.includes("烟草企业，多机构，预算池已知，历史赔付不完整，帮我准备首次拜访材料")\n    ) {';

const index = content.indexOf(targetObj);
if (index === -1) {
    console.log("Could not find the target text.");
    process.exit(1);
}

const endIndex = content.indexOf('return;\n    } else if (\n      text.includes(\n        "根据我上传的分析云南省烟草公司2024-2027年补充医疗保险服务项目（二次）招标书初稿', index);

if (endIndex === -1) {
    console.log("Could not find the end text.");
    process.exit(1);
}

const newContent = `} else if (
      text.includes("烟草企业，多机构，预算池已知，历史赔付不完整，帮我准备首次拜访材料")
    ) {
      const newUserMsg = {
      ...
      }, 500); // speed up animation a bit implicitly
      return;
    } else if (
...
content = content.substring(0, index) + newContent + content.substring(endIndex);
fs.writeFileSync(path, content, 'utf8');
console.log('Successfully updated ChatWindow.tsx');
