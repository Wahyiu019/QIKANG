...
const anchor1 = '} else if (';
const anchor2 = 'text.includes("烟草企业，多机构，预算池已知，历史赔付不完整，帮我准备首次拜访材料")';
const anchor3 = 'const reportMsg = {';
const anchor4 = 'return;\\n    }';

let parts = cwContent.split(anchor2);
if (parts.length === 3) {
  console.log("Found 2 occurrences of the prompt!");
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    if (endIdx === -1) {
      console.log("Could not find endAnchor for occurrence", i);
      return;
    }
    
    let toReplace = suffix.substring(0, endIdx + 15);
    let replacement = newBlockContent + toReplace.replace(/'先定基调。[\s\S]*?说到做到"的文化。',/, 
    ...
    replacedContent += 'text.includes("烟草企业，多机构，预算池已知，历史赔付不完整，帮我准备首次拜访材料"' + replacement + suffix.substring(endIdx + 15);
  }
  
  fs.writeFileSync(cwPath, replacedContent, 'utf8');
  console.log("Patched successfully!");
} else {
  console.log("Expected 2 occurrences, found", parts.length - 1);
}
