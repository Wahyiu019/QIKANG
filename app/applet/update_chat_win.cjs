...
// Inject the states first (for both AiChatWindow and GroupChatWindow):
...
const handlers = `    } else if (text === '#案例亮点提炼#') {
      ...
      setTimeout(() => {
        setMessages(prev => prev.map(m => m.id === reasoningMsgId ? { ...m, data: { ...m.data, steps: m.data.steps.map((s,i)=>i===1?{...s,status:'complete'}:s) } } : m));
        const reportMsg = { id: Date.now().toString() + '_report', sender: 'bot', type: 'case_highlight_report', content: '已经为您完成历史案例的亮点与卖点提炼：', timestamp: new Date() };
      ...
      setTimeout(() => {
        ...
        const reportMsg = { id: Date.now().toString() + '_report', sender: 'bot', type: 'competitor_analysis_report', content: '深度竞品分析报告已生成：', timestamp: new Date() };
        setMessages(prev => [...prev, reportMsg]);
        setIsTyping(false);
      }, 4500);
      return;
      ...
      setMessages(prev => [...prev, newUserMsg]);
      const reasoningMsgId = Date.now().toString();
      const reasoningMsg = { id: reasoningMsgId, sender: 'bot', type: 'reasoning', content: '调用“标品方案生成”技能', timestamp: new Date() };
      setMessages(prev => [...prev, reasoningMsg]);
      setIsTyping(true);
      setTimeout(() => {
      ...
      const reasoningMsg = {
        ...
        data: { steps: [{id:'1', text:'检索标准产品的特征、保障范围、费率等基准信息...', status:'loading'}, {id:'2', text:'获取标准化的营销方案大纲模板...', status:'pending'}] }
      };
      setMessages(prev => [...prev, reasoningMsg]);
      setIsTyping(true);
      setTimeout(() => {
      ...
      setTimeout(() => {
        setMessages(prev => prev.map(m => m.id === reasoningMsgId ? { ...m, data: { ...m.data, steps: m.data.steps.map((s,i)=>i===1?{...s,status:'complete'}:s) } } : m));
        const reportMsg = { id: Date.now().toString() + '_report', sender: 'bot', type: 'standard_product_plan_report', content: '专属标品方案已为您生成：', timestamp: new Date() };
        setMessages(prev => [...prev, reportMsg]);
        setIsTyping(false);
      }, 3000);
      return;
